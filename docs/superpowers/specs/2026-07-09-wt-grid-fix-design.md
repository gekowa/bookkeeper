...
`runWt` 流程：清旧 pidfile → 写启动脚本 → **具名新窗口（`-w bk-<时间戳>`）逐子命令下发**——每步 split 以该 pane 的 pidfile 出现为门控（启动脚本第一行即写 pidfile，pidfile 出现 ⇒ pane 内 PowerShell 已运行 ⇒ 焦点已落定），并顺带采集 PID；`move-focus` 步无门控。门控超时（每 pane 5s）记 `undefined` 并续行，与 0.0.10 语义同构（stop 走端口兜底）。具名窗口同时消除用户 `windowingBehavior` 设定导致粘错窗口的隐患。

> **为何不是单条子命令链**（2026-07-09 裁决补记，commit `99962f9`）：WT 对链式 `split-pane` 的焦点指派与 pane 建立异步竞速，`-File` 载荷真机实测**仅约 1/5 布局正确**；门控逐步下发后 k=6/k=10 全部确定性正确。代价为 `bk start` 增加约 1–4 秒。构造函数相应从拼接单条 argv 的 `buildWtArgs` 改为返回步骤列表的 `buildWtSteps(specs, psHost, scriptFiles): WtStep[]`（`WtStep = { args: string[]; paneIndex?: number }`）。
...
- 改写 `tests/launch/wt.test.ts`：
  - `buildWtSteps` k=1（仅 new-tab）、k=2、k=5、k=6：断言步骤序列（`-V`/`-H`/`--size` 值/`move-focus left` 位置/`-File` 结构/`paneIndex` 列优先构造顺序），断言 args 不含用户命令文本、且完全不含 `;`（逐步下发后链式分隔符不复存在）；
  - 启动脚本内容：pidfile 行 + 命令行、单引号路径转义个案；
  - 服务→pane 指派：k=5（[2,2,1]）验证列优先映射。
- `runWt` 门控循环单测：mock `execa` 以「写对应 pidfile」为副作用——happy path 断言逐步下发（每步一次 `wt` 调用、`-w` 窗口名全程一致）且 PID 按 pane 采集；gate 失败个案（某 pane 不写 pidfile）断言该位 `undefined` 且后续步骤照常下发（fake timers 驱动超时，不耗真实时钟）。
