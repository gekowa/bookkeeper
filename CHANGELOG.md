...
- `bk start` 报 `tmpl.replace is not a function`：`bk_config.yml` 里 `envs` 的标量值（如 `DB_PORT: 15236`，YAML 会推断为 number；布尔、浮点同理）此前未经归一就传给插值器，`.replace` 调用在 number 上崩溃。现 `loadConfig` 统一把 `envs` 值强转为字符串（`.env` 文件语义本就全是字符串），复杂类型（null/对象/数组）改为报 `CONFIG_INVALID`。同时修了 `envs:` 下全为注释时被 YAML 解析为 `null`、原校验逻辑误判通过导致的下游崩溃。
...
- `bk start` 不再裸信任 state 里的运行标志：启动前**探测运行句柄的真实活性**（iTerm 查存活 session unique id、tmux 查存活 pane id）并据此调和。手动关闭承载服务的 iTerm 窗口、进程已全部结束后，`bk start` 会自动清掉陈旧的孤儿运行记录并正常启动，不再误报「已有服务在运行、改用 restart」。服务确实仍在运行时，`bk start` 改为**幂等**：提示「服务已在运行」并正常退出（exit 0），不再以错误退出、也不新开 iTerm 窗口。活性探测带「iTerm 未运行」守卫，绝不因探测把 iTerm 拉起；探测命令异常按「无存活」处理。
